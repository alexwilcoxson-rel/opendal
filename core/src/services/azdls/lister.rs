...
impl AzdlsLister {
    pub fn new(core: Arc<AzdlsCore>, path: String, args: OpList) -> Self {
        Self {
            core,
            path,
            start_after: args.start_after().map(|maybe_with_dir| maybe_with_dir.rsplit_once('/').map_or_else(|| maybe_with_dir.to_string(), |(_, wihout_dir)| wihout_dir.to_string())),
            limit: args.limit(),
        }
    }
}
...
impl oio::PageList for AzdlsLister {
    async fn next_page(&self, ctx: &mut oio::PageContext) -> Result<()> {
        ...
        let first_page = ctx.token.is_empty();
